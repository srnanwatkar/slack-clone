...
function MessagesHeader(props) {
    ...
    const channelName = useSelector(state => state.user_reducer.currentChannel.name);
    ...
    return (
        <Segment clearing>
            <Header fluid='true' as='h2' floated='left' style={{ marginBotton: 0 }}>
                ...
                <Header.Subheader>
                    {props.uniqueGroupUsers !== 0 ? `${props.uniqueGroupUsers} ${props.uniqueGroupUsers === 1 ? 'User' : 'Users'}` : ''}
                </Header.Subheader>
            </Header>
